...
import info.vancauwenberge.idm.util.Messages;
...
public class MarkLocalAction extends LibraryAction
{
  ...
  protected void performAction()
  {
    ...
    Log.error("MarkLocal may only be enabled on a package object");
    return;
  }

  private int markNotImported(){
	  if (!Core.confirmDlg(Messages.getMessage(Messages.MsgKeys.ConfirmLocalize, new String[]{selectedPackage.getName(),selectedPackage.getPackageVersion().getVersion(),selectedPackage.getVendorName()})))
			  return 0;
	  selectedPackage.getVendorName();
	  selectedPackage.setPackageImported(false);
	  String message=Messages.getMessage(Messages.MsgKeys.ChangeLogEntryLocalize, new String[]{selectedPackage.getName(),selectedPackage.getPkgAssociationId(),selectedPackage.getPackageVersion().getVersion()});
	  selectedPackage.addChangeLogEntry(message, false);
	  return 0;
  }
